...
2. Causes: [the publishing filter]{style="color:grey"}
...
## 1. Low prior prob: disease testing
...
## 2. N$_2$O gas emissions from soil
![](images/measuringN2O_1.png){width=80%}

## 2. Spatial variation in N$_2$O emissions
![](images/Crichton_Fn2o_interpolated_June2014_ggmap.png){width=90%}

## 2. We need cumulative emissions
...
# 1. Low prior probability of Land-Use Change {background="#37a635"}
...
# 2. Low statistical power in N$_2$O gas emissions {background="#37a635"}
## Low power e.g. gas emissions
Monte carlo simulation example

## Low power e.g. gas emissions
... estimating the difference in volume under two surfaces:
![](images/time_space_3D_with_points.png){width=45%}
![](images/time_space_3D_with_points_2.png){width=45%}

## Low power e.g. gas emissions
- Propagation of uncertainty via Monte Carlo simulation
    - repeatedly draw samples from given distributions
    - calculate time series of fluxes and accumulate
    - gives the possible distribution of ccumulative emission given data
- Bayesian approach to data analysis
    - propagation of uncertainty is "built in"

# 3. Bias in soil carbon measurement {background="#37a635"}
...
# The publishing filter {background="#37a635"}
